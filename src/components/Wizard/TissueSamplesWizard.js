...
const TissueSamplesWizard = React.memo(({ schema, formData, onSubmit, onBack }) =>  {
    ...
    <Form schema={schema} formData={formData} omitExtraData={true} onSubmit={handleOnSubmit} >
      <div className="footer">
        ...
        <div className="col-xs-5 col-xs-offset-2 submit-panel">
          <button type="submit" className="btn btn-info">Generate Open Minds Documents</button></div>
      </div>
    </Form>
  );
});

export default TissueSamplesWizard;
